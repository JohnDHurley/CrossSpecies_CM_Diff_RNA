---
title: "RNA_Cormotif_Comp_Species_Ensembl"
...
```{r Cormotif Function, message=FALSE, warning=FALSE,include=FALSE}
...
# saveRDS(Filt_RMG0_RNA_log2cpm_NoD4_NoRep,"data/Cormotif/concat/Filt_RMG0_RNA_log2cpm_NoD4_NoRep.RDS")
# saveRDS(RNA_Metadata_NoD4_NoRep,"data/Cormotif/concat/RNA_Metadata_NoD4_NoRep.RDS")
```


```{r RNA-Seq_CorMotif_Species_Modeling}
groupid <- c(rep(1:10,7))
...
# 
# 
# saveRDS(motif.fitted,"data/Cormotif/Species/cormotif_RNA_seed8_k1_15_HC_Day_emmaCode.RDS")

motif.fitted <- readRDS("data/Cormotif/Species/cormotif_RNA_seed8_k1_15_HC_Day_emmaCode.RDS")
plotIC(motif.fitted)
motif.fitted$bic
...
print(motif.fitted$bestmotif$p.post)
...
```{r RNA-Seq_CorMotif_Species_Plotting,fig.height=5,fig.width=9}
...
clust1_RNA <-
  motif_prob %>%
  as.data.frame() %>%
  filter(V1 > 0.59 & V2 <  0.5 & V3 < 0.5 & V4 < 0.5)  %>%
  rownames()
print("clust1_RNA")
length(clust1_RNA)
...
```{r RNA-Seq_CorMotif_Species_ClustSets_Saves,include = FALSE}
# View(motif_prob)
# write.csv(clust1_RNA,"data/Cormotif/Species/clust1_RNA_Species.csv")
# write.csv(clust2_RNA,"data/Cormotif/Species/clust2_RNA_Species.csv")
# write.csv(clust3_RNA,"data/Cormotif/Species/clust3_RNA_Species.csv")
# write.csv(clust4_RNA,"data/Cormotif/Species/clust4_RNA_Species.csv")
# saveRDS(clust1_RNA,"data/Cormotif/Species/clust1_RNA_Species.RDS")
# saveRDS(clust2_RNA,"data/Cormotif/Species/clust2_RNA_Species.RDS")
# saveRDS(clust3_RNA,"data/Cormotif/Species/clust3_RNA_Species.RDS")
# saveRDS(clust4_RNA,"data/Cormotif/Species/clust4_RNA_Species.RDS")
...
saveRDS(gost_results, "data/Cormotif/Species/gost_all_clusters.RDS")
...
write.csv(gost_export,
          "data/Cormotif/Species/gost_all_clusters.csv",
          row.names = FALSE)
```


```{r r RNA-Seq_CorMotif_Species_ClustSets_GO_GProfiler_graphs,fig.height=5, fig.width=7}
...
# Define the order
desired_order <- paste0("clust", 1:5)
...
plot_cluster_trajectories <- function(cluster_genes, TopTable) {
  ...
  ggplot(tt_clust, aes(x = Comparisons, y = logFC, group = Gene)) +
    geom_line(alpha = 0.3, color = "steelblue") +
    geom_point(size = 1, alpha = 0.5, color = "steelblue") +
    scale_y_continuous(limits = c(-15, 15)) +  # manual y-axis range
    theme_minimal(base_size = 14) +
    labs(title = "logFC Trajectories", x = "Comparison", y = "logFC")
}
