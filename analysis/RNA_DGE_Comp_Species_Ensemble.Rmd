...
```{r RNASeq_DGE_Species_TopTable_Subsetting_DEGs0.05_Eulers}
...
```{r RNASeq_DGE_Species_TopTable_Subsetting0.5_barplot}
####proportion bar plots####
#first factor the timepoints so that they are in the right order
df <- data.frame(
  Day = c("Day0", "Day2", "Day5", "Day15", "Day30"),
  DEGs = c(5981,3620,3690,4518,5494),
  Non_DEGs = c(8103,10464,10394,9566,8590)
)
...
```{r RNASeq_DGE_Species_TopTable_Subsetting_DEGs0.1_Eulers}
all_genes <- unique(TopTable_RNA_All$Gene)
...
```{r RNASeq_DGE_Species_TopTable_Subsetting0.1_barplot}
####proportion bar plots####
#first factor the timepoints so that they are in the right order
df <- data.frame(
  Day = c("Day0", "Day2", "Day5", "Day15", "Day30"),
...
```{r RNASeq_DGE_Species_TopTable_Venn_ggplot_plot,fig.height=8,fig.width=8}
...
p <- ggVennDiagram(sets,
  ...
  scale_fill_gradient(low = "skyblue", high = "blue") + # optional color gradient
