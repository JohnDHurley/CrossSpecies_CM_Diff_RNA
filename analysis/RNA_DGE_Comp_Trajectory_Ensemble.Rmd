...
dge <- readRDS("data/DGE/Trajectory/concat/RNA_dge_matrix.RDS")
fit2 <- readRDS("data/DGE/Trajectory/concat/fit2.RDS")
TopTable_RNA_All <- readRDS("data/DGE/Trajectory/concat/TopTable_RNA_all.RDS")
Comparisons_names <- readRDS("data/DGE/Trajectory/concat/Comparisons_names_Trajectory.RDS")
# RNA_sets <- readRDS("data/DGE/Trajectory/concat/RNA_DEG_Non_DEG_sets_05.RDS")
# RNA_sets0.1 <- readRDS("data/DGE/Trajectory/concat/RNA_DEG_Non_DEG_sets_0.1.RDS")
# all_genes <- readRDS("data/DGE/Trajectory/concat/all_genes.RDS")
...
# dim(dge)
# saveRDS(dge, "data/DGE/Trajectory/concat/RNA_dge_matrix.RDS")
# dge$samples
...
# saveRDS(fit2,"data/DGE/Trajectory/concat/fit2.RDS")
```
...
# saveRDS(TopTable_RNA_All,"data/DGE/Trajectory/concat/TopTable_RNA_all.RDS")
...
# saveRDS(Comparisons_names,"data/DGE/Trajectory/concat/Comparisons_names_Trajectory.RDS")
...
# saveRDS(RNA_sets,"data/DGE/Trajectory/concat/RNA_DEG_Non_DEG_sets_05.RDS")
```


```{r RNASeq_DGE_Trajectory_TopTable_Subsetting0.05_Eulers}
all_genes <- unique(TopTable_RNA_All$Gene)
# saveRDS(all_genes,"data/DGE/Trajectory/concat/all_genes.RDS")
...
df <- data.frame(
  ...
  DEGs = c(4849,5325,5996,4655,4435,4286,4852,4369),
  Non_DEGs = c(9235,8759,8088,9429,9649,9798,9232,9715)
)
...
ggplot(df_long, aes(x = Comp, y = Proportion, fill = Type)) +
  ...
  scale_fill_manual(values = c("DEGs" = "darkgreen", "Non_DEGs" = "black")) + # custom colors
  labs(y = "Proportion", x = "Comp", fill = "",title = "Proportion of Trajectory DEGs p < 0.05") +
...
# saveRDS(RNA_sets0.1,"data/DGE/Trajectory/concat/RNA_DEG_Non_DEG_sets_0.1.RDS")
```
...
df <- data.frame(
  Comp = c("H2_0", "H5_2", "H15_5", "H30_15","C2_0","C5_2","C15_5","C30_15"),
  DEGs = c(5837,6387,7048,5692,5420,5144,5825,5432),
  Non_DEGs = c(8247,7697,7036,8392,8664,8940,8259,8652)
)
...
Gene_list <- TopTable_RNA_All$Gene
...
```{r RNASeq_DGE_Trajectory_TopTable_Venn_ggplot_plot,fig.height=10,fig.width=10}
...
write.csv(Inersection_combined_df, "data/DGE/Trajectory/concat/All_Intersections.csv", row.names = FALSE)
```
...
clust_names_list <- readRDS("data/Cormotif/Trajectory/concat/clust_genes,RDS")
TopTable_RNA_All <- readRDS("data/DGE/Trajectory/concat/TopTable_RNA_all.RDS")
...
```{r}
design_int <- model.matrix(~ 0 + RNA_Metadata_NoD4_NoRep$dgelist)
colnames(design_int) <- gsub("RNA_Metadata_NoD4_NoRep\\$dgelist", "", colnames(design_int))
View(design_int)

corfit <- duplicateCorrelation(object = dge$counts, design = design_int, block = RNA_Metadata_NoD4_NoRep$Individual)
v <- voom(dge, design_int, block = RNA_Metadata_NoD4_NoRep$Individual, correlation = corfit$consensus.correlation, plot = TRUE)
fit <- lmFit(v, design_int, block = RNA_Metadata_NoD4_NoRep$Individual, correlation = corfit$consensus.correlation)
...
contrast_matrix <- makeContrasts(
  ...
  C15_C5 = Chimp_Day15 - Chimp_Day5,
  C30_C15 = Chimp_Day30 - Chimp_Day15,
  int_2_0 = (Human_Day2 - Human_Day0) - (Chimp_Day2 - Chimp_Day0),
  int_5_2  = (Human_Day5 - Human_Day2) - (Chimp_Day5 - Chimp_Day2),
  int_15_5 = (Human_Day15 - Human_Day5) - (Chimp_Day15 - Chimp_Day5),
  int_30_15 = (Human_Day30 - Human_Day15) - (Chimp_Day30 - Chimp_Day15),
  levels = design_int)
...
results_summary_int <- decideTests(fit2,
                               adjust.method = "BH",
                               p.value = 0.05)
summary(results_summary_int)
saveRDS(fit2,"data/DGE/Trajectory/fit2_int.RDS")
```
```{r RNASeq_DGE_Trajectory_TopTable}
#Create Human TopTable. using listapply, then you can pull specific comp using the $
TopTables_RNA_names_int <- colnames(fit2$coefficients) 

# colnames(fit2$coefficients)

TopTable_RNA_All_int <- do.call(
  rbind,
  lapply(TopTables_RNA_names_int,function(ct) {
...
TopTable_RNA_All_int$Comparisons
# saveRDS(TopTable_RNA_All,"data/DGE/Trajectory/TopTable_RNA_all.RDS")
head(TopTable_RNA_All_int)
```

```{r RNASeq_DGE_Trajectory_TopTable_VolcancoPlots}
Comparisons_names_int <- unique(TopTable_RNA_All_int$Comparisons)
# saveRDS(Comparisons_names,"data/DGE/Trajectory/Comparisons_names_Trajectory.RDS")

plot <- list()
for (n in Comparisons_names_int) {

plot[[n]] <- TopTable_RNA_All_int %>% 
  dplyr::filter(Comparisons == paste0(n)) %>% 
  generate_volcano_plot(.,n)
}
...
```
```{r}
top_int <- topTable(fit2, coef="int_2_0", number=20)
top_genes <- rownames(top_int)

library(dplyr)
library(tidyr)

expr_mat <- v_int$E[top_genes, ]  # expression for top genes
...
```{r}
top_int <- topTable(fit2, coef="int_5_2", number=20)
top_genes <- rownames(top_int)
...
```{r}
top_int <- topTable(fit2, coef="int_15_5", number=20)
top_genes <- rownames(top_int)
...
```{r}
top_int <- topTable(fit2, coef="int_30_15", number=20)
top_genes <- rownames(top_int)
