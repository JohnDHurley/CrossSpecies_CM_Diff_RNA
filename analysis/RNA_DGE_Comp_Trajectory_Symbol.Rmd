---
title: "RNA_DGE_Comp_Trajectory_Symbol"
...
#Loading R Objects
Filt_RMG0_RNA_fc_NoD4_NoRep <- readRDS("data/Cormotif/Symbol/Filt_RMG0_RNA_fc_NoD4_NoRep.RDS")
...
clust1_RNA <- readRDS("data/Cormotif/Symbol/clust1_RNA_Symbol.RDS")
clust2_RNA <- readRDS("data/Cormotif/Symbol/clust2_RNA_Symbol.RDS")
clust3_RNA <- readRDS("data/Cormotif/Symbol/clust3_RNA_Symbol.RDS")
clust4_RNA <- readRDS("data/Cormotif/Symbol/clust4_RNA_Symbol.RDS")
clust5_RNA <- readRDS("data/Cormotif/Symbol/clust5_RNA_Symbol.RDS")
clust6_RNA <- readRDS("data/Cormotif/Symbol/clust6_RNA_Symbol.RDS")
clust7_RNA <- readRDS("data/Cormotif/Symbol/clust7_RNA_Symbol.RDS")
clust8_RNA <- readRDS("data/Cormotif/Symbol/clust8_RNA_Symbol.RDS")
clust9_RNA <- readRDS("data/Cormotif/Symbol/clust9_RNA_Symbol.RDS")
clust10_RNA <- readRDS("data/Cormotif/Symbol/clust10_RNA_Symbol.RDS")
clust11_RNA <- readRDS("data/Cormotif/Symbol/clust11_RNA_Symbol.RDS")
```
...
dge$samples$group <- factor(RNA_Metadata_NoD4_NoRep$Cond)
dge <- calcNormFactors(dge, method = "TMM")
dge$samples

#dim(dge)
# saveRDS(dge, "data/DGE/Symbol/RNA_dge_matrix.RDS")
#dge$samples
...
# Add Species column
TopTable_RNA_All <- TopTable_RNA_All %>%
  mutate(Species = ifelse(startsWith(Comparisons, "H"), "Human", "Chimp"))

# Get unique species and comparisons
species <- unique(TopTable_RNA_All$Species)
comparisons <- unique(TopTable_RNA_All$Comparisons)

# Loop over species and comparisons
for (sp in species) {
  for (comp in comparisons) {
    
    # Subset the species + comparison
    df_sub <- TopTable_RNA_All %>%
      filter(Species == sp & Comparisons == comp)
    
    # Skip if no rows for this combination
    if (nrow(df_sub) == 0) next
    
    # Significant
    df_sig <- df_sub %>% filter(adj.P.Val < padj_cutoff & abs(logFC) > logFC_cutoff)
    
    # Non-significant
    df_nonSig <- df_sub %>% filter(!(adj.P.Val < padj_cutoff & abs(logFC) > logFC_cutoff))
    
    # Create dynamic names
    sig_name <- paste(sp, comp, "Significant", sep = "_")
    nonSig_name <- paste(sp, comp, "NonSignificant", sep = "_")
    
    # Assign data frames to the global environment
    assign(sig_name, df_sig, envir = .GlobalEnv)
    assign(nonSig_name, df_nonSig, envir = .GlobalEnv)
  }
}
```
...
# Get all Human significant DF names
human_sig_dfs <- ls(pattern = "^Human_.*_Significant$")

venn_plots <- list()

for (human_df_name in human_sig_dfs) {
  
  # Extract comparison suffix by removing "Human_" prefix
  suffix <- sub("^Human_", "", human_df_name)
  
  # Construct corresponding Chimp DF name (replace "H_" with "C_")
  chimp_df_name <- sub("^H_", "C_", suffix)            # "H_D0_D2_Significant" -> "C_D0_D2_Significant"
  chimp_df_name <- paste0("Chimp_", chimp_df_name)     # "Chimp_C_D0_D2_Significant"
  
  # Get gene vectors
  human_genes <- get(human_df_name)$Gene
  chimp_genes <- get(chimp_df_name)$Gene
  
  # Generate Venn diagram
  p <- ggVennDiagram(list(Human = human_genes, Chimp = chimp_genes)) +
    ggtitle(suffix) +
...
# Display one Venn diagram
venn_plots[["H_D0_D2_Significant"]]
venn_plots[["H_D2_D5_Significant"]]
venn_plots[["H_D5_D15_Significant"]]
venn_plots[["H_D15_D30_Significant"]]

# Combine multiple Venn diagrams into a single figure (2 columns)
library(patchwork)
...
```{r}
# All Human significant DF names
human_sig_dfs <- ls(pattern = "^Human_.*_Significant$")

# All Chimp significant DF names
chimp_sig_dfs <- ls(pattern = "^Chimp_.*_Significant$")

shared_DEGs_list <- list()

for (h_df in human_sig_dfs) {
  # Extract suffix, e.g., H_D0_D2
  suffix <- sub("^Human_", "", h_df)
  
  # Find corresponding Chimp DF
  c_df <- grep(suffix, chimp_sig_dfs, value = TRUE)
  
  if (length(c_df) == 0) next  # skip if no match
  
  # Compute overlap
  shared_genes <- get_overlap_genes(h_df, c_df)
  
  # Store in list
  list_name <- paste0("Shared_", suffix)
  shared_DEGs_list[[list_name]] <- shared_genes
}
```
...
```{r RNASeq_DGE_Clusters_FC_Quadrants_2}
# --------------------------
