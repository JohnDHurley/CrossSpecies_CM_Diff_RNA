...
#Loading R Objects
Comparisons_names <- readRDS("data/DGE/Comparisons_names_Trajectory.RDS")
quadrant_gene_lists <- readRDS("data/DGE/quadrant_gene_lists_DayBefore.RDS")
GWAS_CongHD_associations <- readRDS("data/DGE/GWAS_CongHD_associations.RDS")
```
...
```{r Volcano_Plot_GWAS_Function}
generate_volcano_plot_GWAS <- function(toptable, title, gwas_genes = NULL) {
  ...
  # Mark GWAS hits
  toptable <- toptable %>%
    mutate(highlight = if_else(Gene %in% gwas_genes, TRUE, FALSE))

  # Split data for layers
  df_non_gwas <- toptable %>% filter(!highlight)
  df_gwas     <- toptable %>% filter(highlight)
...
```{r RNA_GWAS_DataSets}
# GWAS_CongHD_associations<- read.delim("C:/Users/jdhurley/Downloads/EFO_0005207_associations_export.tsv")
# saveRDS(GWAS_CongHD_associations,"data/DGE/GWAS_CongHD_associations.RDS")

# Make a copy to be safe
GWAS_table_copy <- GWAS_CongHD_associations

# Rename the column directly
colnames(GWAS_table_copy)[colnames(GWAS_table_copy) == "mappedGenes"] <- "Gene"

# Check
colnames(GWAS_table_copy)
# Should include "Gene"

# Assume your GWAS table is called GWAS_table
GWAS_table_long <- GWAS_table_copy %>%
  filter(!is.na(Gene)) %>%                 # remove NAs
  mutate(Gene = str_trim(Gene)) %>%        # trim spaces
  tidyr::separate_rows(Gene, sep = ",") %>% # split comma-separated genes
  mutate(Gene = str_trim(Gene))            # trim again

#saveRDS(GWAS_table_long,"data/DGE/GWAS_table_long.RDS")

dim(GWAS_CongHD_associations)
length(unique(GWAS_table_long$Gene))
unique(GWAS_table_long$Gene)
...
highlight_genes <- inner_join(gene_quadrant_df, GWAS_table_long, by = "Gene")
...
```{r RNA_GWAS_Overlaps}
quadrant_GWAS_overlap <- gene_quadrant_df %>%
  inner_join(
    GWAS_table_long,
    by = "Gene",
    relationship = "many-to-many"
  )
saveRDS(quadrant_GWAS_overlap,"data/DGE/quadrant_GWAS_overlap.RDS")

GWAS_to_clusters <- GWAS_table_long %>%
 inner_join(
   gene_quadrant_df,
   by = "Gene",
   relationship = "many-to-many"
...
GWAS_cluster_summary <- GWAS_to_clusters %>%
 group_by(Gene) %>%
 summarise(
...
GWAS_by_cluster <- GWAS_to_clusters %>%
 group_by(cluster, comparison, quadrant) %>%
 summarise(
   GWAS_genes = list(unique(Gene)),
   n_GWAS_genes = n_distinct(Gene),
   .groups = "drop"
 )

View(quadrant_GWAS_overlap)
unique(quadrant_GWAS_overlap$Gene)
length(unique(quadrant_GWAS_overlap$Gene))
```
...
for (n in Comparisons_names) {   # n is e.g., "H_D0_D2"
  ...
  gwas_genes_n <- highlight_genes_expanded_mapped %>%
    filter(Comparisons == n) %>%
    pull(Gene)

  plots[[n]] <- generate_volcano_plot_GWAS(df_plot, n, gwas_genes_n)
}
...
plot_quadrant_from_topTable <- function(top_table, gene_list, cluster_name, H_comp, C_comp, gwas_genes = NULL) {
  ...
  df_filtered <- df_filtered %>%
    mutate(Significant = (.data[[paste0("adj.P.Val_", H_comp)]] < 0.05 &
           ...
           GWAS_flag = if_else(Gene %in% gwas_genes, TRUE, FALSE))
  ...
  # Split
  df_gwas_sig <- df_filtered %>% filter(GWAS_flag & Significant)
  df_gwas_nsig <- df_filtered %>% filter(GWAS_flag & !Significant)
  df_non_gwas <- df_filtered %>% filter(!GWAS_flag)
  ...
  p <- ggplot() +
    ...
    # Non-significant GWAS
    geom_point(data = df_gwas_nsig,
               aes(x = .data[[paste0("logFC_", H_comp)]],
                   y = .data[[paste0("logFC_", C_comp)]],
                   fill = quadrant_color),
    ...
    # Significant GWAS
    geom_point(data = df_gwas_sig,
               aes(x = .data[[paste0("logFC_", H_comp)]],
                   y = .data[[paste0("logFC_", C_comp)]],
                   fill = quadrant_color),
               shape = 21, color = "limegreen", size = 4, stroke = 1.5) +

    # Labels for significant GWAS
    geom_text_repel(
    data = df_gwas_sig,
    aes(x = .data[[paste0("logFC_", H_comp)]],
        y = .data[[paste0("logFC_", C_comp)]],
...
all_quadrant_results <- lapply(names(cluster_lists), function(clust_name) {
  ...
  lapply(seq_along(H_comparisons), function(i) {
    ...
    # GWAS genes mapped to this cluster
    gwas_genes_i <- highlight_genes_expanded_mapped %>%
      filter(Comparisons %in% c(H_comp, C_comp), Gene %in% genes) %>%
      pull(Gene)
    
    plot_quadrant_from_topTable(TopTable_RNA_All, genes, clust_name, H_comp, C_comp, gwas_genes_i)
  })
})
...
```{r}
clust1_GWAS <- intersect(cluster_lists$clust1,unique(highlight_genes_expanded_mapped$Gene))
clust2_GWAS <- intersect(cluster_lists$clust2,unique(highlight_genes_expanded_mapped$Gene))
clust3_GWAS <- intersect(cluster_lists$clust3,unique(highlight_genes_expanded_mapped$Gene))
clust4_GWAS <- intersect(cluster_lists$clust4,unique(highlight_genes_expanded_mapped$Gene))
clust5_GWAS <- intersect(cluster_lists$clust5,unique(highlight_genes_expanded_mapped$Gene))
clust6_GWAS <- intersect(cluster_lists$clust6,unique(highlight_genes_expanded_mapped$Gene))
clust7_GWAS <- intersect(cluster_lists$clust7,unique(highlight_genes_expanded_mapped$Gene))
clust8_GWAS <- intersect(cluster_lists$clust8,unique(highlight_genes_expanded_mapped$Gene))
clust9_GWAS <- intersect(cluster_lists$clust9,unique(highlight_genes_expanded_mapped$Gene))
clust10_GWAS <- intersect(cluster_lists$clust10,unique(highlight_genes_expanded_mapped$Gene))
clust11_GWAS <- intersect(cluster_lists$clust11,unique(highlight_genes_expanded_mapped$Gene))

```

```{r}
# Create the table
clust2_ChiSq <- matrix(c(21, (2790-21), 7, (1355-7)), nrow = 2, byrow = TRUE)
...
# Create the table
clust3_ChiSq <- matrix(c(21, (2790-21), 7, (459-7)), nrow = 2, byrow = TRUE)
...
# Create the table
clust4_ChiSq <- matrix(c(21, (2790-21), 10, (522-10)), nrow = 2, byrow = TRUE)
...
# Create the table
clust5_ChiSq <- matrix(c(21, (2790-21), 2, (244-2)), nrow = 2, byrow = TRUE)
...
# Create the table
clust6_ChiSq <- matrix(c(21, (2790-21), 3, (427-3)), nrow = 2, byrow = TRUE)
...
# Create the table
clust7_ChiSq <- matrix(c(21, (2790-21), 9, (618-9)), nrow = 2, byrow = TRUE)
...
# Create the table
clust8_ChiSq <- matrix(c(21, (2790-21), 8, (847-8)), nrow = 2, byrow = TRUE)
...
# Create the table
clust9_ChiSq <- matrix(c(21, (2790-21), 3, (596-3)), nrow = 2, byrow = TRUE)
...
# Create the table
clust10_ChiSq <- matrix(c(21, (2790-21), 9, (1041-9)), nrow = 2, byrow = TRUE)
...
# Create the table
clust11_ChiSq <- matrix(c(21, (2790-21), 4, (975-4)), nrow = 2, byrow = TRUE)
...
GWAS_genes <- GWAS_table_long$Gene  # your curated GWAS gene list

quad_counts_gwas <- lapply(names(all_quadrant_results), function(clust_name) {
  ...
  lapply(names(all_quadrant_results[[clust_name]]), function(comp_name) {
    ...
    df %>%
      # Keep only GWAS genes
      filter(Gene %in% GWAS_genes) %>%
      # Quadrant, assign "Zero" if not significant
...
quad_counts_gwas$cluster <- factor(quad_counts_gwas$cluster, levels = paste0("clust", 1:11))
quad_counts_gwas$quadrant_color <- factor(
  quad_counts_gwas$quadrant_color,
...
ggplot(quad_counts_gwas, aes(x = cluster, y = n, fill = quadrant_color)) +
  geom_bar(stat = "identity", position = "fill") +  # percent stacked
  facet_wrap(~comparison) +                        # separate bars per comparison
  ...
  ylab("Percentage of GWAS genes") +
  xlab("Cluster") +
  ggtitle("GWAS genes: Quadrant distribution per cluster and comparison") +
  theme_bw()
...
gwas_cluster_df <- lapply(names(all_quadrant_results), function(clust_name) {
  lapply(names(all_quadrant_results[[clust_name]]), function(comp_name) {
...
```{r}
gwas_cluster_counts <- gwas_cluster_df %>%
  group_by(cluster) %>%
...
```{r}
ggplot(gwas_cluster_df, aes(x = cluster, y = Gene, color = cluster)) +
  geom_point(size = 3) +
  scale_color_brewer(palette = "Paired") +
  labs(
    y = "GWAS gene",
    x = "Motif / Cluster",
    title = "GWAS genes mapped to motifs"
  ) +
  theme_bw() +
