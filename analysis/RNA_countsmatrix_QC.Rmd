...
ggplot(RNA_flagstat, aes(x = row.names(RNA_flagstat), y = Total_Reads/2)) +
  geom_col() +
  geom_hline(yintercept = 20000000, linetype="dotted",color="red",size=1) +
  theme_bw() +
...
```{r Plotting_Boxplot_rawcounts_bytimepoint,fig.height=4,fig.width=6}
...
```{r Plotting_Barplot_MappedReads,fig.height=5,fig.width=10}
ggplot(RNA_flagstat, aes(x = row.names(RNA_flagstat), y = Mapped_Reads/2)) +
  geom_col() +
  geom_hline(yintercept = 20000000, linetype="dotted",color="red",size=1) +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1,vjust=0.4,size=9)) +
  labs(
    title = "Mapped Read Pairs",
    x = "Sample",
    y = "Mapped Read Pairs"
  )
```

```{r Plotting_Boxplot_MappedReads_bytimepoint,fig.height=4,fig.width=6}
ggplot(RNA_flagstat, aes(x = Timepoint, y = Mapped_Reads/2, fill = Species)) +
  geom_boxplot() +
 geom_jitter(aes(fill = Species),
              position = position_jitterdodge(jitter.width = 0.2,
                                              dodge.width = 0.8),
  ...
  theme_bw() +
  labs(
    title = "Mapped Read Pairs by Species (H vs C)",
    x = "Timepoint",
    y = "Mapped Read Pairs"
  )
...
```{r Plotting_Barplot_ProperlyPaired_bytimepoint,fig.height=4,fig.width=6}
...
ggplot(RNA_flagstat, aes(x = Timepoint, y = Properly_Paried/2, fill = Species)) +
 ...
 geom_jitter(aes(fill = Species),
  ...
  theme_bw() +
  labs(
    title = "Properly Paried Read Paires",
    x = "Timepoint",
    y = "Properly Paired Read Pairs"
  )
